...
These interaction methods are both *read-only*. In addition to the
command data you see in normal output there is a `version`
key that indicates the current version of TooLoo that is producing the
data, and a `status` key, which corresponds to an [HTTP
status code](https://developer.mozilla.org/en-US/docs/Web/HTTP/Status).

If an error is encountered there will also be an `error` key
...
-   `show-json <command_name>` Generates JSON with the
    details of the command. Contains the same information you\'d see
    from the `show` command but in JSON format.
...
-   `list-json` Generates JSON with the name and short
...
Hostname can be `localhost`, `127.0.0.1`,
`0.0.0.0`, or any other valid hostname in your system. Port
is optional and defaults to `6996`
...
TooLoo uses SQLite under the covers, and as such only one process can
access the database at a time. This means that there is technically a
small possibility that if a request comes in from the command line AND
from the web server at the same time, the 2nd process will get an error
about the DB being locked.

However TooLoo is a single user tool, so YOU would have to use the web
server AND the command line at exactly the same moment, which would be
very impressive, and if you manage to do it we want video. ;)
